...
def create_comparison_plots(results, training_times, config, logger, naive_baseline_cost, models):
    ...
    plt.tight_layout()
    plt.savefig('utils/plots/comparative_study_results_gpu_fast.png', dpi=300, bbox_inches='tight')
    logger.info("📊 Comparison plots saved to utils/plots/comparative_study_results_gpu_fast.png")
...
def create_performance_table(results, training_times, param_counts, logger, config):
    ...
    df.to_csv('utils/plots/comparative_results_gpu_fast.csv', index=False)
    logger.info("📋 Detailed results saved to utils/plots/comparative_results_gpu_fast.csv")
...
def save_results(results, training_times, models, config):
    ...
    torch.save({
        ...
        "config": config
    }, "pytorch/comparative_study_gpu_fast_complete.pt")

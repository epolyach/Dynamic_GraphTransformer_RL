...
class DynamicGraphTransformerUltra(nn.Module):
    """Ultra-lightweight Dynamic Graph Transformer optimized for performance.
    
    Key optimizations:
    - Balanced hidden dimension (112) for good expressiveness
    - 3 attention layers for better representation
    - 4 attention heads for rich attention patterns
    - Enhanced state encoding (3 features)
    - Target: ~70K parameters (5x smaller than GAT+RL)
    """
    def __init__(self, input_dim, hidden_dim, num_heads, num_layers, dropout, feedforward_multiplier, config):
        super().__init__()
        # Optimized settings for ~30K parameters (between Super and Lite)
        self.hidden_dim = 48   # Reduced for 30K target
        self.num_heads = 3     # Divisible attention heads
        self.num_layers = 2    # Same as Super for efficiency
        ...
        # Enhanced state encoding (3 features: capacity, visit progress, step)
        self.state_encoder = nn.Linear(3, self.hidden_dim // 4)  # 12 dims
        
        # Enhanced dynamic update with deeper network
    ...
    def _generate_routes_dynamic(self, node_embeddings, node_features, demands_batch, capacities,
        ...
        for step in range(max_steps):
            ...
            # Enhanced state features (capacity, visit progress, step progress)
            ...
            state_features = torch.stack([capacity_used, visited_count, step_progress], dim=1)  # [batch, 3]
            state_encoding = self.state_encoder(state_features)  # [batch, 12]
            
            # Enhanced dynamic update
            state_context = state_encoding.unsqueeze(1).expand(-1, max_nodes, -1)  # [batch, nodes, 12]
            update_input = torch.cat([node_embeddings, state_context], dim=-1)  # [batch, nodes, 48+12=60]
            updated_embeddings = self.dynamic_update(update_input)  # [batch, nodes, 48]

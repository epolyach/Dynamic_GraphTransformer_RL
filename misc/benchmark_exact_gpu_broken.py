    ...
    for i, (instance, results) in enumerate(zip(instances, batch_results)):
        ...
        for solver_name in solver_names:
            ...
            if result and result["success"]:
                ...
                if debug:
                    seed = 4242 + n_customers * 1000 + i * 10  # Reconstruct seed used for this instance
                    if result.get("vehicle_routes"):
                        routes_str = ', '.join([str(route) for route in result["vehicle_routes"]])
                    else:
                        routes_str = 'N/A'
                    print(f"🐛 DEBUG [{solver_name}] Instance {i+1}/{len(instances)}, N={n}, Seed={seed}")
                    print(f"   └─ CPC: {cpc:.6f}, Cost: {cost:.6f}, Routes: [{routes_str}]")

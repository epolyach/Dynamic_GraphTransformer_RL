...
class MatlabLogger:
    ...
    def _write_header(self):
        ...
        self.file_handle.write(f"% CPU CVRP Benchmark - MATLAB Format Log\n")
        self.file_handle.write(f"% Date: {timestamp}\n")
        self.file_handle.write(f"% Config: capacity=30, demand=[1,10]\n")
    ...
    def log_instance_start(self, n, instance, instance_num):
        ...
        if not self.should_log_instance(n):
            return
        
        self.instance_counts[n] = self.instance_counts.get(n, 0) + 1
        
        # Format problem data
        coords = instance["coords"]
        demands = instance["demands"]
        capacity = instance.get("capacity", 30)
        ...
        self.file_handle.write(f"% capacity = {capacity};\n")
        self.file_handle.flush()
    
    def log_solver_result(self, n, solver_name, solution):
        """Log a solver's result."""
        if not self.should_log_instance(n) or not solution:
            return

...
# General script to run GPU training configurations sequentially with PREFETCH
# Usage: ./run_seq_prefetch.sh config1.yaml config2.yaml config3.yaml ...
# Example: ./run_seq_prefetch.sh configs/tiny_gpu_150.yaml configs/tiny_gpu_500.yaml &
...
SCRIPT_PATH="training_gpu_prefetch/scripts/run_training_gpu.py"
MODEL="GT+RL"
LOG_FILE="sequential_training_prefetch_$(date +%Y%m%d_%H%M%S).log"

# Create log file
echo "Sequential Training Log (PREFETCH) - Started at $(date)" > "$LOG_FILE"
...
run_experiment() {
    ...
    local session_name="prefetch_${exp_name}_$$"
    ...
    screen -dmS "$session_name" bash -c "
        cd $(pwd) &&
        source $VENV_PATH && 
        echo 'Starting PREFETCH training for $config_file at \$(date)' &&
...
# Main execution
log_msg "Starting sequential PREFETCH training of $# configurations"
log_msg "Process ID: $$"
...
log_msg "Results can be found in training_gpu_prefetch/results/"
log_msg "Full log available in: $LOG_FILE"

# Summary
echo ""
echo "=========================================="
echo "SEQUENTIAL TRAINING SUMMARY (PREFETCH)"
echo "=========================================="
...
for i in $(seq 1 $#); do
    ...
    echo "  - training_gpu_prefetch/results/$exp_name/"
done

#!/usr/bin/env python3
"""
Plot Exact CVRP Solver Benchmark Results
Creates publication-quality plots from exact solver benchmark CSV files.
"""
...
def create_exact_solver_plots(csv_file: str, output_prefix: str = "exact_benchmark", title: str = None):
    """
    ...
    if title:
        ...
        fig.suptitle('Exact CVRP Solver Performance Comparison', 
    ...
    print("\n" + "="*60)
    print("🔍 EXACT SOLVER BENCHMARK SUMMARY")
    print("="*60)
    ...
    for solver_key, solver_label, _, _ in solvers:
        ...
        if differences:
            ...
            status = "✅ EXCELLENT" if abs(avg_diff) < 0.01 else "⚠️  SUBOPTIMAL" if abs(avg_diff) < 1.0 else "❌ POOR"
            print(f"• {solver_label:15} vs OR-Tools: {avg_diff:+.3f}% avg, {max_diff:+.3f}% max, {agreement:.0f}% agree {status}")


def main():
    parser = argparse.ArgumentParser(description='Plot Exact CVRP Solver Benchmark Results')
    parser.add_argument('csv_file', help='CSV file with benchmark results')
    parser.add_argument('--output', default='exact_benchmark', help='Output prefix for plot files')
    parser.add_argument('--title', help='Custom title for the plot')
    
    args = parser.parse_args()
    
    create_exact_solver_plots(args.csv_file, args.output, args.title)

...
from training_cpu.lib.rollout_baseline import RolloutBaseline
...
def _scale_beta2_for_batch_size(config: Dict[str, Any], logger_print: Callable) -> float:
    """Compute scaled beta2 using β2_new = (β2_base)^(bs_new / bs_base)."""
    opt_cfg = config.get('optimizer_experiments', {})
    base_beta2 = float(opt_cfg.get('base_beta2', 0.999))
    bs_new = int(config['training']['batch_size'])
    bs_base = int(opt_cfg.get('base_batch_size', 512))

    scaled_beta2 = base_beta2 ** (bs_new / bs_base)
    logger_print(f"[OptimizerExp] AdamW beta2 scaling: base_beta2={base_beta2}, bs_base={bs_base}, bs_new={bs_new} -> beta2_new={scaled_beta2:.8f}")
    return scaled_beta2


def advanced_train_model_opt(
    model: nn.Module, 
    model_name: str, 
    config: Dict[str, Any],
    ...
    """
    Experimental trainer variant for optimizer experiments.
    Changes from the main trainer are minimal and isolated:
    - Optional AdamW beta2 scaling as β2_new = (β2_base)^(bs_new/bs_base)
    - Optional advantage normalization control to avoid zero gradients for bs=1
    All other logic (rollout baseline, LR scheduling, etc.) is identical.
    """
    ...
    # Optimizer experiment config
    opt_cfg = config.get('optimizer_experiments', {})
    override_beta2 = bool(opt_cfg.get('override_beta2', False))
    adv_norm_method = str(opt_cfg.get('normalize_advantages', 'batch')).lower()  # 'batch' | 'none'
    adv_norm_eps = float(opt_cfg.get('eps', 1e-8))
    ...
    # Adaptive temperature scheduler (reusing same simple logic from base trainer)
    temp_scheduler = None
    if use_adaptive_temp:
        from training_cpu.lib.advanced_trainer import AdaptiveTemperatureScheduler
    ...
    logger_print(f"[{model_name}] (OPT) Training with {num_instances} total instances over {num_epochs + 1} epochs")
    logger_print(f"[{model_name}] (OPT) Batches per epoch: {batches_per_epoch}, batch size: {batch_size}")
    
    start_time = time.time()
    
    # Set up optional rollout baseline (identical behavior)
    ...
    for epoch in range(0, num_epochs + 1):
        ...
        for batch_idx in range(batches_per_epoch):
            ...
            for b in range(len(instances)):
                ...
                if strict_validation:
                    validate_route(r, len(instances[b]['coords']) - 1, f"{model_name}-TRAIN-OPT", instances[b])
            ...
            if optimizer is not None and model_name != 'GT-Greedy':
                ...
                # Advantage normalization (experimental control)
                if adv_norm_method == 'none':
                    adv_norm = advantages
                else:
                    adv = advantages
                    adv_mean = adv.mean()
                    adv_std = adv.std()
                    if adv_std < adv_norm_eps:
                        # Avoid zero gradients for tiny batches; fall back to raw advantages
                        adv_norm = adv
                    else:
                        adv_norm = (adv - adv_mean) / (adv_std + adv_norm_eps)
        ...
        if (epoch % config['training']['validation_frequency']) == 0 or epoch == num_epochs:
            with torch.no_grad():
                ...
                for b in range(len(val_instances)):
                    r = routes_g[b]
                    if strict_validation:
                        validate_route(r, len(val_instances[b]['coords']) - 1, f"{model_name}-VAL-OPT", val_instances[b])
                ...
                # Temperature update if using adaptive
                if temp_scheduler:
        ...
        if early_stopping is not None and val_cost is not None:
            if early_stopping(val_cost, model):
                logger_print(f"[{model_name}] (OPT) Early stopping triggered at epoch {epoch}")
                early_stopping.restore_best_model(model)
                break
        
        # Compute baseline value for CSV logging only
        baseline_value = None
        if epoch_costs:
        ...
        if val_cost is not None:
            logger_print(
                f"[{model_name}] (OPT) Epoch {epoch:03d}: "

...
logger.warning(f"Failed to load model file {fname}: {e}")
continue
...
training_times[mname] = m['training_time']
 ...
 for name in missing:
  logger.warning(f"Skipping {name} (no model file / parameters found)")
...
logger.warning(f"Failed to read CSV for {model_name}: {e}")
return None
  # Ensure required columns exist (best-effort)
  for col in ['epoch', 'train_loss', 'train_cost', 'val_cost']:
if col not in df.columns:
 logger.warning(f"CSV for {model_name} missing column '{col}'")

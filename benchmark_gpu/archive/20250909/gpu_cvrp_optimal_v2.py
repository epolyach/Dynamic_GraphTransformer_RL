...
class GPUOptimalCVRPv2:
    ...
    def __init__(self, device='cuda', mode=SolverMode.DYNAMIC_PROGRAMMING, 
        ...
        if torch.cuda.is_available():
            ...
            self.max_threads = 1024  # Common value for modern GPUs
            self.max_blocks = self.gpu_properties.max_gridsize[0]
            self.shared_memory_per_block = self.gpu_properties.shared_memory_per_block
            print(f"GPU: {self.gpu_properties.name}")
            print(f"Max threads/block: {self.max_threads}")
            print(f"Shared memory/block: {self.shared_memory_per_block/1024:.1f} KB")

...
Including the actual OR-Tools results for N=100
"""
...
# Load OR-Tools N=100 results (100 instances)
try:
    with open('ortools_gls_n100_100inst_1.0s_20250905_114444.json', 'r') as f:
        d = json.load(f)
        data['heur_100'] = compute_full_stats(d['all_cpcs'])
        data['heur_100']['method'] = 'OR-Tools GLS'
except FileNotFoundError:
    pass

# Add data for N=20 and N=50 from the partial run
# Based on partial run: N=20: GM=0.332016, GSD=1.150870
...
print("\\item AD: Anderson-Darling test statistic (critical value at 5\\% = 0.787; * indicates rejection)")
print("\\item * Based on partial benchmark data from GPU heuristic")
print("\\item OR-Tools GLS: 100 instances with 1s time limit per instance")

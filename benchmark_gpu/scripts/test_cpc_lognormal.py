...
def load_cpc_data(data_file):
    """Load CPC data from CSV or JSON file."""
    ...
    if file_ext == '.csv':
        ...
        if 'cpc' not in df.columns:
            print(f"Error: 'cpc' column not found in {data_file}")
        ...
        if 'all_cpcs' not in data:
            print(f"Error: 'all_cpcs' field not found in {data_file}")
            print(f"Available fields: {list(data.keys())}")
            sys.exit(1)
        
        cpc_data = data['all_cpcs']
        if not isinstance(cpc_data, list) or len(cpc_data) == 0:
            print(f"Error: 'all_cpcs' should be a non-empty list in {data_file}")
            sys.exit(1)
            
        return np.array(cpc_data)
    
    else:
        print(f"Error: Unsupported file format '{file_ext}'. Supported formats: .csv, .json")
        sys.exit(1)
...
def main():
    ...
    parser.add_argument('data_file', nargs='?', 
                        help='CSV or JSON file containing CPC data (default: auto-detect latest CSV)')
    ...
    if args.data_file is None:
        ...
        if not csv_files:
            ...
            print("Please provide a CSV or JSON file as argument.")
            sys.exit(1)
        args.data_file = sorted(csv_files)[-1]  # Get the latest file
        print(f"Auto-detected CSV file: {args.data_file}")
    
    # Determine file type
    file_ext = os.path.splitext(args.data_file)[1].lower()
    print(f"Loading CPC data from {file_ext.upper()} file: {args.data_file}")
    
    # Load data
    cpc_data = load_cpc_data(args.data_file)

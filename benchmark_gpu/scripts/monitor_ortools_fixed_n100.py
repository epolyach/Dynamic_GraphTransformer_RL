#!/usr/bin/env python3
"""
Fixed continuous monitor for OR-Tools GLS benchmarks
Corrected N=100 display issue - only show 10k instance run, not 1k
"""
...
def check_process_status():
    ...
    # Check N=100 - ONLY the 10k instance run
...
def get_progress_info():
    ...
    # Check N=100 progress from 10k log only
    log_file_100 = 'ortools_10000inst.log'
    if os.path.exists(log_file_100):
        try:
            with open(log_file_100, 'r') as f:
                lines = f.readlines()
                for line in reversed(lines):
                    if 'Progress:' in line:
                        parts = line.strip().split(' - ')
                        if len(parts) >= 3:
                            prog_part = parts[0].split(': ')[1]
                            gm_part = parts[1].split(': ')[1]
                            eta_part = parts[2].split(': ')[1]
...
def collect_available_results():
    ...
    # Check N=100 - ONLY 10k instances (ignore 1k)
...
def generate_ortools_table(results):
    ...
    for n in sorted(results.keys()):
        ...
        row = f"{n} & {data['instances']} & {stats['gm']:.4f} & {stats['gsd']:.4f} & "
...
def display_status(results, running, progress):
    ...
    print("="*80)
    print("OR-TOOLS GLS BENCHMARKS - CONTINUOUS MONITOR (CORRECTED N=100)")
    print("="*80)
...
def monitor_loop():
    ...
    print("Starting OR-Tools GLS continuous monitor (CORRECTED)...")
    ...
    try:
        while True:
            ...
            if current_result_count > last_result_count and results:
                ...
                # Also save as latest
                with open('ortools_gls_latest_corrected.tex', 'w') as f:
                    f.write(latex_table)
                
                print("✓ Latest table saved: ortools_gls_latest_corrected.tex")
                print()
        ...
        print(f"Latest table available in: ortools_gls_latest_corrected.tex")

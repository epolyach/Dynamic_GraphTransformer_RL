...
- SE(Mean) = s / sqrt(n)
- GM = exp(mean(log X)); SE(GM) = GM * log(GSD) / sqrt(n) where GSD = exp(sd(log X))
- SE(Median) via KDE at the sample median m_hat: 1 / (2 f_hat(m_hat) sqrt(n))
- 95% Range percentiles = [2.5th, 97.5th] percentiles of CPC
- KS tests: p-values only, for CPC and log(CPC)

Usage:
  python3 generate_latex_comparison.py --input gpu_dp_exact_results_20250908_104018.json --method "GPU-DP-Exact" --timeout "∞"
"""
...
def load_json_data(json_path: Path):
    with open(json_path, 'r') as f:
        data = json.load(f)
    # Expect specific keys consistent with your other scripts
    if 'all_cpcs' not in data:
        raise KeyError("Missing key 'all_cpcs' in JSON")
    if 'n_customers' not in data:
        raise KeyError("Missing key 'n_customers' in JSON")
    if 'capacity' not in data:
        raise KeyError("Missing key 'capacity' in JSON")
    return data['all_cpcs'], data['n_customers'], data['capacity']

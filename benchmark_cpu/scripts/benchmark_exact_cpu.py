...
Compares exact_ortools_vrp, exact_dp (N≤10 only), and heuristic_or solvers
...
class MatlabLogger:
    ...
    def log_solver_result(self, n, solver_name, solution):
        ...
        if 'exact_dp' in solver_name.lower():
        ...
        # Write result with updated format including "cost/route:"
        self.file_handle.write(f"{solver_display} cost/route: {cpc:.4f} {route}\n")
        self.file_handle.flush()
...
def calculate_route_cost(vehicle_routes: List[List[int]], distances: np.ndarray) -> float:
    ...
    IMPORTANT: This function assumes routes include depot nodes (0) at start and end.
    It calculates:
    - Cost from depot to first customer
    - All customer-to-customer costs
    - Cost from last customer back to depot
    """
    total_cost = 0.0
    for route in vehicle_routes:
        # Each route should include depot nodes, e.g., [0, 1, 2, 3, 0]
        for i in range(len(route) - 1):
            # Add distance from route[i] to route[i+1]
            # This includes depot->first customer and last customer->depot
...
def validate_solutions(ortools_solution: CVRPSolution, other_solutions: Dict[str, CVRPSolution], 
    ...
    if validation_errors:
        ...
        for solver_name in ['exact_dp', 'heuristic_or']:
...
def run_benchmark_for_n(n, instances_min, instances_max, capacity, demand_range, total_timeout, coord_range, logger, matlab_logger, csv_writer, disabled_solvers=None, debug=False):
    ...
    for i in range(instances_max):
        ...
        for attempt in range(MAX_INSTANCE_ATTEMPTS):
            ...
            for solver_name, solver_module in solvers.items():
                # Skip exact_dp for N>10 (too expensive)
                if solver_name == "exact_dp" and n > 10:
                    instance_solutions[solver_name] = None
                    logger.info(f"exact_dp: SKIPPED (N>10, too expensive for brute force)")
                    continue
    ...
    for solver in ['exact_dp', 'exact_ortools_vrp', 'heuristic_or']:
...
def main():
    ...
    try:
        ...
        safe_print(f"📝 MATLAB format log: {matlab_log_file}")

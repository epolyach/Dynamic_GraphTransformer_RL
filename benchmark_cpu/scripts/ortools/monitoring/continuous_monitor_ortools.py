#!/usr/bin/env python3
"""
Continuous monitor for all OR-Tools GLS benchmarks
Automatically generates tables as results become available
"""
...
def display_status(results, running):
    """Display current status in terminal"""
    os.system('clear')
    
    print("="*80)
    print("OR-TOOLS GLS BENCHMARKS - CONTINUOUS MONITOR")
    print("="*80)
    ...
    if not running:
        ...
        for key, pids in running.items():
            if key == '100_1k':
                ...
                print(f"N={key:3d} (10k inst):  PIDs: {', '.join(pids)}")
    print()
    ...
    # Show estimates for running processes
    if running:
        print("TIME ESTIMATES FOR RUNNING PROCESSES:")
        print("-"*50)
        for key in running.keys():
            if key == '100_1k':
                print("N=100 (1k inst):   ETA ~17 minutes total")
            elif key == '100_10k':
                print("N=100 (10k inst):  ETA ~5.5 hours total")  
            elif key in [10, 20, 50]:
                print(f"N={key:3d} (10k inst):  ETA ~5.5 hours total")
        print()
...
def monitor_loop():
    ...
    print("Starting OR-Tools GLS continuous monitor...")
    ...
    try:
        while True:
            ...
            # Display status
            current_result_count = display_status(results, running)
        ...
        print(f"Final table available in: ortools_gls_latest.tex")

...
from .batch_prefetcher import BatchPrefetcher, DummyPrefetcher
...
def advanced_train_model_gpu(
    ...
    if resume_checkpoint is not None:
        logger.info(f"Restoring training state from checkpoint (epoch {resume_checkpoint['epoch']})")
        ...
        # Restore scheduler state (if using scheduling)
        if scheduler is not None and 'scheduler_state_dict' in resume_checkpoint:
            scheduler.load_state_dict(resume_checkpoint['scheduler_state_dict'])
            logger.info("  ✓ Scheduler state restored")
        
        logger.info(f"Checkpoint restoration complete. Resuming from epoch {start_epoch}")
        
        # Note: Baseline will be restored after it's initialized (see below)
    ...
    # Check if we can skip baseline initialization when resuming
    skip_baseline_init = (resume_checkpoint is not None and 
                          'baseline_state_dict' in resume_checkpoint and
                          resume_checkpoint['baseline_state_dict'] is not None)
    
    if skip_baseline_init:
        print("[INIT] Resuming from checkpoint - baseline will be restored from saved state")
        print("[INIT] Skipping expensive baseline initialization...")
        baseline = None  # Will be restored after model/optimizer restoration
    elif use_baseline:
        ...
        # Restore baseline state from checkpoint if resuming
        if resume_checkpoint is not None and 'baseline_state_dict' in resume_checkpoint:
            if resume_checkpoint['baseline_state_dict'] is not None and baseline is not None:
                try:
                    baseline.load_state_dict(resume_checkpoint['baseline_state_dict'])
                    logger.info("  ✓ Baseline state restored")
                except Exception as e:
                    logger.warning(f"  ⚠ Could not restore baseline state: {e}")
    
    # Handle case where we skipped baseline init - create baseline from checkpoint
    if skip_baseline_init and use_baseline:
        print("[INIT] Creating baseline from checkpoint state...")
        # Create a minimal eval dataset (will be overridden by loaded state)
        eval_batches = 1  # Minimal - just for initialization
        eval_dataset = []
        seed_val = 123456
        batch_data = data_generator(batch_size, seed=seed_val)
        gpu_batch = [move_to_gpu_except_distances(inst, gpu_manager) for inst in batch_data]
        ...
        # Create baseline with minimal initialization
        baseline = RolloutBaselineGPU(
            gpu_manager=gpu_manager,
            model=model,
            eval_dataset=eval_dataset,
        ...
        try:
            baseline.load_state_dict(resume_checkpoint['baseline_state_dict'])
            print(f"[INIT] ✓ Baseline restored from checkpoint (no expensive re-evaluation)")
            logger.info(f"  ✓ Baseline state restored from checkpoint")
        except Exception as e:
            logger.error(f"  ✗ Failed to restore baseline state: {e}")
            raise
    ...
    if resume_checkpoint is not None and 'history' in resume_checkpoint:
        history = resume_checkpoint['history']
        logger.info(f"Restored history with {len(history.get('train_loss', []))} epochs")
    else:
        history = {
            'train_loss': [],
    ...
    # Training loop
    num_steps = train_config.get('num_steps', 150)
    # Match CPU semantics: include epoch 0, so epochs are (num_epochs + 1)
    total_epochs = n_epochs + 1
    total_instances = train_config.get('num_instances', total_epochs * num_steps * batch_size)
    print(f"[{model_name}] Training with {total_instances} total instances over {total_epochs} epochs")
    print(f"[{model_name}] Batches per epoch: {num_steps}, batch size: {batch_size}")
    if start_epoch > 0:
    ...
    for epoch in range(start_epoch, total_epochs):        
        ...
        enable_prefetch = data_gen_config.get('enable_prefetch', False)
        prefetch_queue_size = data_gen_config.get('prefetch_queue_size', 3)
        
        if enable_prefetch:
            logger.info(f"🚀 Using data prefetching with queue_size={prefetch_queue_size}")
            prefetcher_class = BatchPrefetcher
        else:
            logger.info("Using sequential data generation (no prefetch)")
            prefetcher_class = DummyPrefetcher
        
        seed_base = epoch * n_batches * batch_size
        
        with prefetcher_class(
            data_generator=data_generator,
            batch_size=batch_size,

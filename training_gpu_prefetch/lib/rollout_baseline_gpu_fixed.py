...
class RolloutBaselineGPU:
    ...
    def __init__(
        ...
        self.strict_validation = config.get('experiment', {}).get('strict_validation', True)
    ...
    def _compute_batch_costs(self, model: torch.nn.Module, instances: List[Dict[str, Any]]) -> np.ndarray:
        ...
        if hasattr(self, 'gpu_manager'):
            instances = [self.gpu_manager.to_device_dict(inst, non_blocking=True) for inst in instances]
        """
    ...
    def _compute_dataset_costs(self, model: torch.nn.Module) -> np.ndarray:
        ...
        for batch_idx, batch in enumerate(self.eval_dataset):
    ...
    def state_dict(self) -> Dict[str, Any]:
        """Return baseline state for checkpointing."""
        return {
            'baseline_model_state': self.baseline_model.state_dict() if hasattr(self, 'baseline_model') and self.baseline_model is not None else None,
            'bl_vals': self.bl_vals if hasattr(self, 'bl_vals') else None,
            'mean': self.mean if hasattr(self, 'mean') else float('inf'),
            'epoch': self.epoch if hasattr(self, 'epoch') else 0,
            'eval_dataset_size': len(self.eval_dataset) if hasattr(self, 'eval_dataset') else 0
        }
    
    def load_state_dict(self, state: Dict[str, Any]) -> None:
        """Restore baseline state from checkpoint."""
        if 'baseline_model_state' in state and state['baseline_model_state'] is not None:
            if hasattr(self, 'baseline_model') and self.baseline_model is not None:
                self.baseline_model.load_state_dict(state['baseline_model_state'])
        
        if 'bl_vals' in state and state['bl_vals'] is not None:
            self.bl_vals = state['bl_vals']
        ...
        self.logger_print(
            f"[RolloutBaseline] Restored from checkpoint: epoch={self.epoch}, mean={self.mean:.6f}"
        )
    
    @property
    def best_val(self) -> float:
        """Return current baseline mean (for compatibility)."""
        return self.mean if hasattr(self, 'mean') else float('inf')

...
class IncrementalCSVWriter:
    ...
    def __init__(self, model_name: str, base_dir: Path, logger):
        ...
        # Initialize CSV with headers only if file doesn't exist
        if not self.csv_path.exists():
            ...
            self.logger.info(f'Using existing CSV history file: {self.csv_path}')
    
    def write_epoch(self, history: dict, epoch: int):
        """Write data for a single epoch from history dict."""
        # The epoch parameter is the actual epoch number (e.g., 61, 62, 63...)
        # But history is 0-indexed, so after training 61 epochs (0-60), 
        # epoch 60's data is at history[60], not history[61]
        # We need to use the LAST item in history, not epoch as index!
        
        hist_len = len(history.get('train_cost_arithmetic', []))
        if hist_len == 0:
            self.logger.warning(f"Cannot write epoch {epoch}: history is empty")
            return
        
        # Use the last index in history (most recent epoch that was just completed)
        idx = hist_len - 1
        
        row = {
            'epoch': epoch,
            'train_loss': history.get('train_loss', [])[idx] if idx < len(history.get('train_loss', [])) else None,
            'train_cost_arithmetic': history.get('train_cost_arithmetic', [])[idx],
            'val_cost_arithmetic': history.get('val_cost_arithmetic', [])[idx] if idx < len(history.get('val_cost_arithmetic', [])) else None,
            'learning_rate': history.get('learning_rate', [])[idx] if idx < len(history.get('learning_rate', [])) else 1e-4,
            'temperature': history.get('temperature', [])[idx] if idx < len(history.get('temperature', [])) else 2.5,
            'time_per_epoch': history.get('epoch_time', [])[idx] if idx < len(history.get('epoch_time', [])) else 0.0,
            'baseline_type': history.get('baseline_type', [])[idx] if idx < len(history.get('baseline_type', [])) else '',
            'baseline_value': history.get('baseline_value', [])[idx] if idx < len(history.get('baseline_value', [])) else float('nan'),
            'mean_type': history.get('mean_type', [])[idx] if idx < len(history.get('mean_type', [])) else 'arithmetic'
        }
        
        self.rows.append(row)
        pd.DataFrame([row]).to_csv(self.csv_path, mode='a', header=False, index=False)
        self.logger.info(f"✓ Wrote epoch {epoch} to CSV")
...
def train_single_model(model_name: str, config: Dict[str, Any], args, logger):
    ...
    if args.resume:
        ...
        resume_checkpoint, start_epoch = load_checkpoint_for_resume(args.resume, logger)

        # Override config with checkpoint config if needed
    ...
    try:
        ...
        logger.error(f"CRITICAL FAILURE: Training cannot continue. Problem size={config.get('problem', {}).get('num_customers', 'N/A')},")
        logger.error(f"  batch_size={config.get('training', {}).get('batch_size', 'N/A')}, lr={config.get('training', {}).get('learning_rate', 'N/A')}")
        # Re-raise to propagate error - scientific code must fail loudly
        raise RuntimeError(f"Training failed for {model_name} (n={config.get('problem', {}).get('num_customers', '?')})") from e
    finally:
...
def load_checkpoint_for_resume(checkpoint_path: str, logger) -> Tuple[Dict[str, Any], int]:
    """
    ...
    Returns:
        Tuple of (checkpoint_dict, start_epoch)
    """
    import torch
    from pathlib import Path
    ...
    start_epoch = epoch + 1  # Resume from next epoch
    
    logger.info(f"Checkpoint loaded successfully:")
    logger.info(f"  - Saved at epoch: {epoch}")
    logger.info(f"  - Will resume from epoch: {start_epoch}")
    logger.info(f"  - Contains: {list(checkpoint.keys())}")
    
    if 'history' in checkpoint:
        history = checkpoint['history']
        logger.info(f"  - Training history has {len(history.get('train_loss', []))} epochs")
...
Examples:
  ...
  python %(prog)s --model DGT+RL --device cuda:1 --output-dir results/my_experiment
    ...
    if len(results) == 0:
        logger.error("CRITICAL: All training runs failed. No successful results.")
        return 1
    
    if len(results) < len(models_to_train):
        logger.warning(f"WARNING: {len(models_to_train) - len(results)} out of {len(models_to_train)} models failed to train")
        logger.warning("Partial failure detected - returning error code")
        return 1

...
class BaselinePointerNetwork(nn.Module):
    """Simple Pointer Network with basic attention.
    Mirrors the inline implementation used in the project.
    """
    ...
    def __init__(self, input_dim: int, hidden_dim: int, config=None):
        ...
        input_multiplier = config['model']['pointer_network']['input_multiplier'] if config else 2
    ...
    def forward(self, instances, max_steps, temperature, greedy, config):
        ...
        return self._generate_routes(attended, node_features, demands_batch, capacities,
    ...
    def _generate_routes(self, node_embeddings, node_features, demands_batch, capacities,
        ...
        for step in range(max_steps):
            # Check per-batch completion (all customers visited AND at depot)
            for b in range(batch_size):
                if not batch_done[b]:
            ...
            # Context: mean of unvisited nodes
            ...
            # Sample actions for unfinished batches

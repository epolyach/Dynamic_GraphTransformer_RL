...
class GraphAttentionTransformer(nn.Module):
    """Graph Attention Transformer with edge-agnostic attention (as inline in project)."""
    def __init__(self, input_dim, hidden_dim, num_heads, num_layers, dropout, edge_embedding_divisor, config):
        ...
        gat_config = config['model']['graph_attention_transformer'] if config else {'input_multiplier': 2, 'output_dim': 1}
            ...
            for b in range(batch_size):
                if not batch_done[b]:
                    if greedy:
                        ...
                        probs_b = torch.softmax(scores[b] / temperature, dim=-1)

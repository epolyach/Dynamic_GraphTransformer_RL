...
class GraphTransformerLite(nn.Module):
    """Lightweight Graph Transformer optimized for RL training.
    
    Key simplifications:
    - Uses MultiheadAttention directly (no TransformerEncoderLayer)
    - No heavy feedforward expansion 
    - Minimal layer normalization
    - Reduced parameter count (~365K similar to GAT)
    """
    def __init__(self, input_dim, hidden_dim, num_heads, num_layers, dropout, feedforward_multiplier, config):
        super().__init__()
        # Override to match DGT-Lite settings (~100K parameters)
        self.hidden_dim = 84
        self.num_heads = 3
        self.num_layers = 3
        self.node_embedding = nn.Linear(input_dim, self.hidden_dim)
        
        # Lightweight attention layers (no feedforward expansion)
        self.attention_layers = nn.ModuleList([
            nn.MultiheadAttention(self.hidden_dim, self.num_heads, dropout=dropout, batch_first=True)
            for _ in range(self.num_layers)
        ])
        
        # Minimal normalization (only where necessary)
        self.layer_norms = nn.ModuleList([
            nn.LayerNorm(self.hidden_dim) for _ in range(self.num_layers)
        ])
        
        # Simple global context aggregation
        self.global_attention = nn.MultiheadAttention(self.hidden_dim, self.num_heads, batch_first=True)
        
        # Simple pointer network (GAT-style)
        context_multiplier = config['model']['pointer_network']['context_multiplier'] if config else 2
        self.pointer = nn.Sequential(
            nn.Linear(self.hidden_dim * context_multiplier, self.hidden_dim),
            nn.ReLU(),
    ...
    def forward(self, instances, max_steps=None, temperature=None, greedy=False, config=None):
        ...
        for attention, norm in zip(self.attention_layers, self.layer_norms):
            ...
            x = norm(x + attn_out)  # Simple residual connection
        
        # Global context aggregation (similar to GAT)
    ...
    def _generate_routes(self, node_embeddings, node_features, demands_batch, capacities, 
        ...
        for step in range(max_steps):
            ...
            # Simple global context (GAT-style)

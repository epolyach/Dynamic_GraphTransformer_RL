...
class DynamicGraphTransformer(nn.Module):
    ...
    def forward(self, instances: List[Dict], max_steps: Optional[int] = None,
        ...
        for i, inst in enumerate(instances):
            ...
            coords_tensor = inst['coords'].detach().clone().to(dtype=torch.float32, device=device) if isinstance(inst['coords'], torch.Tensor) else torch.tensor(inst['coords'], dtype=torch.float32, device=device)
            demands_tensor = inst['demands'].detach().clone().to(dtype=torch.float32, device=device) if isinstance(inst['demands'], torch.Tensor) else torch.tensor(inst['demands'], dtype=torch.float32, device=device)

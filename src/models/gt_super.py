...
class GraphTransformerSuper(nn.Module):
    """Super-lightweight Graph Transformer matching DGT-Super parameters.
    
    Key optimizations:
    - Ultra-small hidden dimension (32) to match DGT-Super
    - 2 attention heads for minimal viable attention
    - 2 layers for basic representation learning
    - Target: ~10K parameters (matching DGT-Super exactly)
    """
    def __init__(self, input_dim, hidden_dim, num_heads, num_layers, dropout, feedforward_multiplier, config):
        super().__init__()
        # Override to match DGT-Super settings (~10K parameters)
        self.hidden_dim = 32
        self.num_heads = 2
        self.num_layers = 2

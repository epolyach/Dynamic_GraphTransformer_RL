...
class DynamicGraphTransformerLite(nn.Module):
    """Lightweight Dynamic Graph Transformer optimized for RL training.
    
    Key optimizations:
    - Moderate hidden dimension (80) for good balance
    - 3 attention layers with 3 heads each
    - Enhanced state encoding (4 features)
    - Streamlined dynamic updates with residual connections
    - Target: ~100K parameters (3.6x smaller than GAT+RL)
    """
    def __init__(self, input_dim, hidden_dim, num_heads, num_layers, dropout, feedforward_multiplier, config):
        super().__init__()
        # Optimized settings for ~100K parameters
        self.hidden_dim = 84   # Divisible by 3 heads
        self.num_heads = 3     # Balanced attention heads
        self.num_layers = 3    # Good depth for representation
        ...
        # Enhanced state encoding (4 features: capacity, visit, step, remaining demand)
        self.state_encoder = nn.Linear(4, self.hidden_dim // 3)  # 26-27 dims
        
        # Enhanced dynamic update with residual connection
        self.dynamic_update = nn.Sequential(
            nn.Linear(self.hidden_dim + self.hidden_dim // 3, self.hidden_dim),
            nn.ReLU(),
    ...
    def forward(self, instances, max_steps=None, temperature=None, greedy=False, config=None):
        ...
        # Lightweight attention layers with residual connections
    ...
    def _generate_routes_dynamic(self, node_embeddings, node_features, demands_batch, capacities,
        ...
        for step in range(max_steps):
            ...
            # Enhanced state features (4 features)
            capacity_used = (capacities - remaining_capacity) / capacities
            visited_count = visited.float().sum(dim=1) / max_nodes
            step_progress = torch.full((batch_size,), step / max_steps, dtype=torch.float32)
            remaining_demand = torch.sum(demands_batch * (~visited).float(), dim=1) / torch.sum(demands_batch, dim=1).clamp(min=1)
            
            state_features = torch.stack([capacity_used, visited_count, step_progress, remaining_demand], dim=1)
            state_encoding = self.state_encoder(state_features)  # [batch, hidden//3]
            
            # Enhanced dynamic update with residual
            state_context = state_encoding.unsqueeze(1).expand(-1, max_nodes, -1)  # [batch, nodes, hidden//3]
            update_input = torch.cat([node_embeddings, state_context], dim=-1)  # [batch, nodes, hidden + hidden//3]
            delta = self.dynamic_update(update_input)  # Enhanced transform
            updated_embeddings = node_embeddings + 0.2 * delta  # Residual connection

...
exec(open("gpu_optimizations.py").read())
...
if device.type == 'cuda':
        if epoch == 0:  # GPU OPTIMIZATION: Monitor GPU memory usage
...
def train_model_gpu(model, instances, config, model_name, logger):
    ...
    # GPU OPTIMIZATION: Enable mixed precision and memory optimizations
    ...
    for epoch in range(config['num_epochs']):
        ...
        # GPU OPTIMIZATION: Increase batch size for GPU efficiency
        batch_size = min(config['batch_size'] * 4, 32)  # 4x larger batches for GPU
        ...
        for batch_idx in range(num_batches):
            ...
            optimizer.zero_grad()
            # GPU OPTIMIZATION: Use mixed precision for forward pass
            ...
            # Replace the original forward pass
            # routes, log_probs = model(batch_instances, temperature=config['temperature'])
            
            routes, log_probs = model(batch_instances, temperature=config['temperature'])
            
            # GPU OPTIMIZED: Batch cost computation
            distance_matrices = [inst['distances'] for inst in batch_instances]
            n_customers_list = [len(inst['coords']) - 1 for inst in batch_instances]
            
            # Fast validation (simplified for GPU efficiency)
            valid_routes = validate_routes_gpu(routes, n_customers_list)
            
            # GPU-accelerated cost computation
            try:
                costs_tensor = compute_batched_route_costs_gpu(routes, distance_matrices, device)
            except Exception as e:
                # Fallback to CPU computation if GPU fails
                print(f"GPU cost computation failed: {e}, falling back to CPU")
                costs = []
...
def run_comparative_study():
    ...
    config = {
        ...
        'num_heads': 4,
        'num_layers': 2,
        'batch_size': max(args.batch, 16),  # GPU OPTIMIZATION: Minimum batch size 16
    ...
    models = {
        'Pointer+RL': PointerNetworkRL(3, config['hidden_dim']),
...
def create_comparison_plots(results, training_times, config, logger, naive_baseline_cost, models):
    ...
    plt.tight_layout()
    plt.savefig('utils/plots/utils/plots/comparative_study_results_gpu.png', dpi=300, bbox_inches='tight')
...
def create_performance_table(results, training_times, param_counts, logger, config):
    ...
    df.to_csv('utils/plots/utils/plots/comparative_results_gpu.csv', index=False)
...
def save_results(results, training_times, models, config):
    ...
    torch.save({
        ...
        "config": config
    }, "pytorch/comparative_study_gpu_complete.pt")

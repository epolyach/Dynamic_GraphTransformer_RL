#!/usr/bin/env python3
"""
Create log-normal distribution figure for CVRP instances.
Final journal-quality version with specific formatting requirements.
Modified to accept destination folder via CLI.
"""
import argparse
import json
import os
...
def load_cpc(data_file):
    ...
    if not os.path.exists(data_file):
        ...
        dir_path = os.path.dirname(data_file) if os.path.dirname(data_file) else '.'
        filename = os.path.basename(data_file)
        available_files = [f for f in os.listdir(dir_path) if f.endswith(('.json', '.csv'))]
        
        error_msg = f"File not found: {data_file}\n"
        if available_files:
            error_msg += "Available files in directory:\n"
            for f in sorted(available_files):
                error_msg += f"  - {f}\n"
        else:
            error_msg += "No .json or .csv files found in directory.\n"
...
def main():
    p = argparse.ArgumentParser(description='Make log-normal distribution figure for CVRP instances (85mm x 65mm)')
    p.add_argument('data_file', help='CSV or JSON file with CPC data (cpc column or all_cpcs field)')
    p.add_argument('dest_folder', help='Destination folder to save the histogram files')
    p.add_argument('--bins', type=int, default=40, help='Number of histogram bins (default: 40)')
    p.add_argument('--out-prefix', type=str, default='log_norm_histogram', help='Output file prefix')
    args = p.parse_args()
    ...
    try:
        ...
        # Check for valid CPC data
        if len(cpc) == 0:
            raise ValueError("No CPC data found in file")
        
        # Filter out non-positive values for log transformation
        positive_cpc = cpc[cpc > 0]
        if len(positive_cpc) == 0:
            raise ValueError("No positive CPC values found for log transformation")
        
        if len(positive_cpc) < len(cpc):
            print(f"Warning: Filtered out {len(cpc) - len(positive_cpc)} non-positive CPC values")
        
        log_cpc = np.log(positive_cpc)
        ...
        # Save with requested filenames in the destination folder
        png_file = os.path.join(args.dest_folder, f"{args.out_prefix}.png")
        eps_file = os.path.join(args.dest_folder, f"{args.out_prefix}.eps")
        ...
        print(f"  - Data points: {len(positive_cpc)}")
        print(f"  - Log-normal params: μ={mu:.3f}, σ={sigma:.3f}")
